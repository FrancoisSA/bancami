# Bancami — Manuel utilisateur v1.1
...
- Tous les messages Telegram sont préfixés **[BANCAMI]** pour identification immédiate
- `/resume` affiche désormais le **reste disponible** (budget global − consommé)
- Un résumé `/resume` est envoyé **automatiquement après chaque import** de relevé
- Extraction des dates corrigée : format français JJ/MM/AAAA reconnu explicitement, année courante injectée dynamiquement pour éviter les erreurs d'inférence
...
├── extractor.py        # Extraction Claude Vision (dates FR JJ/MM/AAAA)
